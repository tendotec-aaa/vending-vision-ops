-- Update the process_visit_report_inventory function to use original_toy_capacity for old toy
...
BEGIN
    ...
    LOOP
        ...
        IF v_is_replacing AND v_original_product_id IS NOT NULL AND v_replacement_product_id IS NOT NULL THEN
            ...
            -- Use original_toy_capacity for OLD toy, fall back to toy_capacity if not present
            ...
            -- Insert movement for OLD toy being removed (uses original_toy_capacity)
            ...
            VALUES (
                ...
                0, v_last_stock, v_original_toy_capacity,  -- Use original_toy_capacity for old toy
                'Toy replaced with ' || COALESCE(v_replacement_product_name, 'new product')
            );

            -- Insert movement for NEW toy being installed (uses new toy_capacity)
            INSERT INTO public.toy_movements (
                visit_report_id, visit_report_date, company_id, company_name,
                location_id, location_name, spot_id, spot_name, user_id,
                machine_id, slot_number, product_id, product_name,
            ...
            VALUES (
                ...
                v_new_toy_refill, 0, v_new_toy_capacity,  -- Use new toy_capacity for new toy
